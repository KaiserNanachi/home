...
## ①SENTIMENT ANALYSIS(古典派)

[**シリコンバレー銀を破綻に追い込んだ現代の取り付け騒ぎ、ツイッターがパニックあおる**](https://www.cnn.co.jp/tech/35201318.html)

**↑上記記事に興味が湧き実際に可視化することに**

1.rtweet packageでSVBに関する東部標準時3/6\~3/11日の間に投稿されたツイートをスクレイピング
...
・yが大きいとposivite,小さいとnegative感情である。

```{r}

fit <- smooth.spline(sent$num, sent$ave, df = 15)
plot(sent$num, sent$ave)
lines(fit, col = "blue")

```

・↑二枚目のプロットに関して、感情時系列に波がある→恐怖と楽観が交互に訪れる。恐怖や不安もバックラッシュを起こす?
